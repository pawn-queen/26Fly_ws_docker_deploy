...
当前正式配置使用 Jetson 源码仓库中由 Git 同步的两份 TensorRT engine：

- `/workspace/src/detect/models/26fly_jetson.engine`
- `/workspace/src/fly/models/26fly_jetson.engine`

detect 与 control 分别使用所属包目录中的模型。切换为本目录中的独立模型时，修改 `config/runtime.env`：
